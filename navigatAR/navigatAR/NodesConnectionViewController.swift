...
class MapOverlay: NSObject, MKOverlay {
	...
	init(floorPlan: IAFloorPlan, andRotatedRect rotated: CGRect) {
		...
		let topLeft = MKMapPoint.init(floorPlan.topLeft)
		boundingMapRect = MKMapRect.init(x: topLeft.x + Double(rotated.origin.x), y: topLeft.y + Double(rotated.origin.y), width: Double(rotated.size.width), height: Double(rotated.size.height))
	}
}
...
//			self.map!.setCamera(newCam, animated: true)
			let viewRegion = MKCoordinateRegion.init(center: floorPlan!.center, latitudinalMeters: Double(floorPlan!.widthMeters)/5, longitudinalMeters: Double(floorPlan!.heightMeters)/5)
	...
	func removeConnection(to nodeCircle: NodeCircle) {
		...
		connections.forEach({ connection in
			if connection.1.MKAnnotation == nodeCircle.MKAnnotation {
				map.removeOverlay(connection.0)
